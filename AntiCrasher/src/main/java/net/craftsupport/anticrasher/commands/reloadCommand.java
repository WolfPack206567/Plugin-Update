...
public class reloadCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (command.getName().equalsIgnoreCase("acreload")) {
            if (!commandSender.hasPermission("anticrasher.command.reload")) {
                commandSender.sendMessage(ChatColor.RED + "You don't have permission to use this command.");
                return true;
            }
            commandSender.sendMessage(ChatColor.GREEN + "Anticrasher successfully reloaded!");
            plugin.reloadConfig();
            util.reloadConfig();
            plugin.getLogger().info("AntiCrasher Has Reloaded!");
